...
class APIClient {
    ...
    func send<T: Codable>(_ request: APIRequest) async throws -> T {
        ...
        guard let url = URL(string: baseURL.appending(request.endpoint.appendingPath)) else {
            Current.logger.critical("Failed to form a baseURL from \(self.baseURL.appending(request.endpoint.appendingPath), privacy: .auto)")
        ...
        print("⬆️ Sending: \(String(describing: urlRequest.allHTTPHeaderFields)) \n to \(String(describing: urlRequest.url))")
